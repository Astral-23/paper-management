...
| `closeEditModal`で論文編集モータルが閉じる | 1. `editModal`を表示状態にする<br>2. `closeEditModal`を実行 | `editModal`が非表示状態になる |
...
### 1. テスト計画
- **テスト種別**: 結合テスト
- **目的と範囲**: 論文カード上のボタンクリックによるステータス変更機能が、Firebaseとの連携を含めて正しく動作することを検証する。ユーザーのクリックから、Firebaseの更新関数呼び出し、成功時のトースト表示までを範囲とする。実際のFirestore通信はモックする。
- **テスト対象**:
    - `script.js` (イベント委譲によるリスナー部分)
    - `firebase.js` (`changePaperStatus`)
    - `ui.js` (`showToast`)

### 2. テスト設計
| テストの具体的な内容 | 操作手順 | 期待結果 |
| :--- | :--- | :--- |
| 論文カードのステータス変更ボタンをクリックすると、更新処理が呼ばれ、成功通知が表示される | 1. テスト用の論文を準備し、UIに描画する。<br>2. `firebase.js`の`changePaperStatus`関数が成功するようにモックする。<br>3. 論文カード内のステータス変更ボタン (`button[data-action="change-status"]`) をクリックする。 | 1. `changePaperStatus`が、対象論文のIDを引数として呼び出されること。<br>2. `changePaperStatus`が成功した後、`ui.showToast`が成功メッセージと共に呼び出されること。 |
| ステータス変更処理が失敗した場合、エラー通知が表示される | 1. テスト用の論文を準備し、UIに描画する。<br>2. `firebase.js`の`changePaperStatus`関数が失敗（Reject）するようにモックする。<br>3. 論文カード内のステータス変更ボタンをクリックする。 | 1. `changePaperStatus`が呼び出されること。<br>2. `ui.showToast`がエラーメッセージと共に呼び出されること。 |
...
### 1. テスト計画
- **テスト種別**: 結合テスト
- **目的と範囲**: 論文の削除機能が正しく動作することを検証する。削除ボタンのクリック、確認ダイアログの表示、Firebaseの削除関数の呼び出し、成功時のトースト表示までを範囲とする。
- **テスト対象**:
    - `script.js` (イベント委譲リスナー)
    - `firebase.js` (`deletePaper`)
    - `ui.js` (`showToast`)
    - `window.confirm` (グローバル)

### 2. テスト設計
| テストの具体的な内容 | 操作手順 | 期待結果 |
| :--- | :--- | :--- |
| 削除ボタンをクリックし、確認ダイアログで「はい」を選択すると、削除処理が呼ばれ、成功通知が表示される | 1. テスト用の論文をUIに描画する。<br>2. `window.confirm`が`true`を返すようにモックする。<br>3. `firebase.js`の`deletePaper`関数が成功するようにモックする。<br>4. 論文カード内の削除ボタン (`button[data-action="delete-paper"]`) をクリックする。 | 1. `window.confirm`が呼び出されること。<br>2. `deletePaper`が、対象論文のIDを引数として呼び出されること。<br>3. `deletePaper`が成功した後、`ui.showToast`が成功メッセージと共に呼び出されること。 |
| 削除ボタンをクリックし、確認ダイアログで「いいえ」を選択すると、処理が中断される | 1. テスト用の論文をUIに描画する。<br>2. `window.confirm`が`false`を返すようにモックする。<br>3. `firebase.js`の`deletePaper`関数をスパイする。<br>4. 論文カード内の削除ボタンをクリックする。 | 1. `window.confirm`が呼び出されること。<br>2. `deletePaper`が呼び出されないこと。 |
| 削除処理が失敗した場合、エラー通知が表示される | 1. テスト用の論文をUIに描画する。<br>2. `window.confirm`が`true`を返すようにモックする。<br>3. `firebase.js`の`deletePaper`関数が失敗（Reject）するようにモックする。<br>4. 論文カード内の削除ボタンをクリックする。 | 1. `deletePaper`が呼び出されること。<br>2. `ui.showToast`がエラーメッセージと共に呼び出されること。 |
...
### 1. テスト計画
- **テスト種別**: 結合テスト
- **目的と範囲**: フォームからの論文追加機能が正しく動作することを検証する。フォームへの入力、送信、Firebaseの追加関数の呼び出し、成功時のトースト表示とフォームのリセットまでを範囲とする。
- **テスト対象**:
    - `script.js` (フォーム送信イベントリスナー)
    - `firebase.js` (`addPaper`)
    - `ui.js` (`showToast`)

### 2. テスト設計
| テストの具体的な内容 | 操作手順 | 期待結果 |
| :--- | :--- | :--- |
| フォームに有効なデータを入力して送信すると、論文が追加され、成功通知が表示される | 1. `firebase.js`の`addPaper`関数が成功するようにモックする。<br>2. フォームの各入力フィールドにテストデータを入力する。<br>3. フォームを送信する。 | 1. `addPaper`が、フォームの内容を反映した正しいデータで呼び出されること。<br>2. `ui.showToast`が成功メッセージと共に呼び出されること。<br>3. フォームがリセットされること。 |
| タイトルが空のままフォームを送信すると、エラー通知が表示され、追加処理は呼ばれない | 1. `firebase.js`の`addPaper`関数をスパイする。<br>2. タイトル入力フィールドを空のまま、フォームを送信する。 | 1. `addPaper`が呼び出されないこと。<br>2. `ui.showToast`がエラーメッセージ（「タイトルは必須です」など）と共に呼び出されること。 |
| 追加処理が失敗した場合、エラー通知が表示される | 1. `firebase.js`の`addPaper`関数が失敗（Reject）するようにモックする。<br>2. フォームに有効なデータを入力して送信する。 | 1. `addPaper`が呼び出されること。<br>2. `ui.showToast`がエラーメッセージと共に呼び出されること。 |
...
### 1. テスト計画
- **テスト種別**: 結合テスト
- **目的と範囲**: 論文編集モーダルを開き、フォームを更新し、保存する機能が正しく動作することを検証する。編集ボタンのクリック、モーダルの表示、フォームの更新、保存ボタンのクリック、Firebaseの更新関数の呼び出し、成功時のトースト表示までを範囲とする。
- **テスト対象**:
    - `script.js` (イベント委譲リスナー、モーダル関連リスナー)
    - `ui.js` (`openEditModal`, `closeEditModal`, `showToast`)
    - `firebase.js` (`updatePaper`)

### 2. テスト設計
| テストの具体的な内容 | 操作手順 | 期待結果 |
| :--- | :--- | :--- |
| 編集ボタンをクリックすると、モーダルが開き、フォームに論文データが設定される | 1. テスト用の論文をUIに描画する。<br>2. 論文カード内の編集ボタン (`button[data-action="open-edit-modal"]`) をクリックする。 | 1. `ui.openEditModal`が対象論文のIDを引数に呼び出されること。<br>2. 編集モーダルが表示状態になること。<br>3. モーダル内の各入力フィールドに、論文の元のデータが設定されていること。 |
| フォームを編集して保存すると、更新処理が呼ばれ、成功通知が表示される | 1. 編集モーダルを開く。<br>2. `firebase.js`の`updatePaper`関数が成功するようにモックする。<br>3. モーダル内のタイトル入力フィールドの値を変更する。<br>4. 保存ボタン (`#save-edit-btn`) をクリックする。 | 1. `updatePaper`が、更新されたデータで呼び出されること。<br>2. `ui.showToast`が成功メッセージと共に呼び出されること。<br>3. 編集モーダルが非表示になること。 |
| 編集フォームのタイトルを空にして保存すると、エラー通知が表示される | 1. 編集モーダルを開く。<br>2. `firebase.js`の`updatePaper`関数をスパイする。<br>3. タイトル入力フィールドを空にする。<br>4. 保存ボタンをクリックする。 | 1. `updatePaper`が呼び出されないこと。<br>2. `ui.showToast`がエラーメッセージと共に呼び出されること。<br>3. 編集モーダルは表示されたままであること。 |